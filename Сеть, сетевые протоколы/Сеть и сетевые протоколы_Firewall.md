# Домашнее задание к занятию "`Сеть и сетевые протоколы: Firewall`" - `Семенов С.В.`
...
6. Для vm1: настройте статический IP адрес у интерфейса смотрящего в сеть 192.168.123.0 (адрес в сети 123.0 выберите на своё усмотрение). Настройте IP-адрес интерфейса смотрящего в вашу локальную сеть так, чтобы был доступен Интернет. Проверьте доступность по ICMP c vm1 до 8.8.8.8.
...
настроил соединение с помощью имеющихся ВМ:
masterubu - 192.168.56.103
vm1 - 192.168.56.107
